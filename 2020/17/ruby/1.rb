...
def process_input_file(filename)
  ...
  }.flatten(1).select { |coord| coord != nil }.map { |coord| [coord, 1] }.to_h
end
...
def all_neighbours(grid, dimensions)
  grid.keys.map { |coord| neighbours(coord, dimensions) }.flatten(1).to_set.to_a
end
...
def alive_cubes(grid, dimensions, iterations)
  ...
  (1..iterations).each { |iteration|
    next_grid = Hash.new
    all_spaces = (all_neighbours(grid, dimensions) + grid.keys).to_set.to_a
    ...
    all_spaces.each { |coord|
      ...
      elsif !grid.key?(coord) && active_neighbours == 3
        next_grid[coord] = 1
      end
    }
    grid = next_grid
